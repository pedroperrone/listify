defmodule Listify.Query do
  ...
  def order(queriable, attribute, order \\ :asc) do
    order_by(queriable, [{^order, ^attribute}])
  end

  def custom_filters(queriable, filters),
    do: Enum.reduce(filters, queriable, &custom_query/2)
